...
public class CteProtocol extends DefaultProtocol {
	...
	protected Set<Action> buildActionSet(SUT system, State state) throws ActionBuildException{
		...
		String someText = "alsdfjl43j5l34jlsajdlfjasl;dfjlasjdflasjdlfjsdalalsdfjsu345ll3j45rljl43j5l34jlsajdlfjasl;dfjlasjdflasjdlfjsdal" +
				"fjsu345ll3j45rljl43j5l34jlsajdlfjasl;dfjlasjdflasjdlfjsdalalsdfjsu345ll3j45rljl43j5l34jlsajdlfjasl;dfjlasjdflasjdlfjsdal";
		...
		for(Widget w : state){
			...
			if(w.get(Enabled, true) && !w.get(Blocked, false)){
				if(!Role.isOneOf(role, UIASeparator, UIAToolBar, UIAToolTip, UIAMenuBar, 
						...
						UIATitleBar, UIAThumb, UIAEdit, UIAText) &&  !(w.get(Title, "").matches("Cte(Classification|Composition|Class)"))){
					...
					if(!title.matches(settings().get(ConfigTags.ClickFilter))){					
						...
						// click on the right part of UIASplitButtons
						if(role.isA(UIARoles.UIASplitButton) && Util.hitTest(w, 0.8, 0.5))
							actions.add(ac.leftClickAt(w, 0.8, 0.5));
					}
				}
			}
			...
			// right clicks
			if(w.get(Enabled, true) && !w.get(Blocked, false)){
				if((w.get(Title, "").matches("Cte(Classification|Composition|Class)") || Role.isOneOf(role, UIARoles.UIATree))){
					if(Util.hitTest(w, 0.5, 0.5)){
			...
			// typing
			if(w.get(Enabled, true) && !w.get(Blocked, false)){
				if(Role.isOneOf(role, UIAEdit, UIAText) && (w.get(UIATags.UIAIsKeyboardFocusable) || w.get(UIATags.UIAHasKeyboardFocus)) && !(w.get(Title, "").matches("CteTree"))){
					if(Util.hitTest(w, 0.5, 0.5)){
						actions.add(ac.clickTypeInto(w, someText));
					}
				}
			}
